...
$_DATOS_EXCEL[$i]['asesores'] = $objPHPExcel->getActiveSheet()->getCell('B' . $i)->getCalculatedValue();
...
foreach ($_DATOS_EXCEL as $key => $value)
{
	...
	$dni    = $value['dni'];
	$asesores   = $value['asesores'];

	if (strlen($value['dni'])>0) 
	{

    echo Asesores::agregar(	$dni,$asesores);
